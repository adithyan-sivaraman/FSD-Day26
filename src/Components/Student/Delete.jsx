...
const DeleteStudent = () => {
    ...
    const handleProceed = async()=>{
        const response = await fetch(`${mockAPIUrl}/student/${studentId}`, {
            method: "DELETE",
            headers: {
        ...
        setTimeout(()=>{
            setFormData((prevData)=>prevData.filter(student=>student.id!==studentId));
            setDialogHidden('');
        },2000)
    }
    ...
    return (
        <div className="flex flex-col w-full ml-2 mt-2">
            <p className="px-4 text-lg font-bold tracking-wider">Delete Student</p>
